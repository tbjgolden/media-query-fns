...
test("toEnglishString each feature", () => {
  ...
  expect(
    toEnglishString(
      compileQuery("(orientation: portrait) and (orientation: landscape)")
    )
  ).toEqual("if is square");
  expect(toEnglishString(compileQuery("(aspect-ratio: 1/1)"))).toEqual(
    "if is square"
  );
  expect(toEnglishString(compileQuery("(max-aspect-ratio: 2/1)"))).toEqual(
    "if aspect-ratio ≤ 2:1"
  );
  expect(toEnglishString(compileQuery("(min-aspect-ratio: 1/2)"))).toEqual(
    "if 1:2 ≤ aspect-ratio"
  );
  expect(toEnglishString(compileQuery("(min-aspect-ratio: 2/1)"))).toEqual(
    "if 2:1 ≤ aspect-ratio"
  );
  ...
  expect(toEnglishString(compileQuery("(update: slow)"))).toEqual(
    "if layout should update slowly"
  );
  ...
  expect(toEnglishString(compileQuery("not (device-aspect-ratio)"))).toEqual(
    "never"
  );
